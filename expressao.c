...
double evaluatePostfix(const char *expressao) {
    ...
    while (*token) {
        ...
        if (isdigit(buffer[0]) || (buffer[0] == '-' && isdigit(buffer[1]))) {
            ...
            double a = pop(&pilha);
            if (a < 0) {
                printf("Erro: Raiz quadrada de n√∫mero negativo!\n");
                exit(1);
            }
            push(&pilha, sqrt(a));
            ...
            double a = pop(&pilha);
            if (a <= 0) {
                printf("Erro: Logaritmo de n√∫mero menor ou igual a zero!\n");
                exit(1);
            }
            push(&pilha, log10(a));
        } else {
...
int validarExpressaoPostfixada(const char *expressao) {
    int operandoCount = 0;
    int operadorCount = 0;
    int encontrouEspaco = 0; // Para verificar se h· espaÁos
    ...
    while (*token) {
        if (isspace(*token)) {
            encontrouEspaco = 1; // H· pelo menos um espaÁo
            token++;
            continue;
        }
        ...
        if (isdigit(buffer[0]) || (buffer[0] == '-' && isdigit(buffer[1]))) {
            ...
            if (isOperadorUnario(buffer)) {
                if (operandoCount < 1) {
                    printf("Erro: Operador un·rio '%s' sem operandos suficientes.\n", buffer);
                    return 0;
                }
            } else {
                if (operandoCount < 2) {
                    printf("Erro: Operador bin·rio '%s' sem operandos suficientes.\n", buffer);
                    return 0;
                }
                operandoCount--; // Dois operandos viram um na pilha
            }
        } else {
            printf("Erro: Token inv·lido encontrado '%s'.\n", buffer);
            return 0;
        }
    }

    // Verifica se h· espaÁos na express„o
    if (!encontrouEspaco) {
        printf("Erro: A express„o deve conter espaÁos para separar operandos e operadores.\n");
        return 0;
    }
